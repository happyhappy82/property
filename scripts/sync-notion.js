...
function findExistingFileByPageId(pageId) {
  const files = fs.readdirSync(PROPERTIES_DIR).filter(file => file.endsWith('.md'));
...
function deletePropertyFile(slug) {
  const filePath = path.join(PROPERTIES_DIR, `${slug}.md`);
...
async function scheduledSync() {
  ...
  console.log(`📚 Found ${response.results.length} published properties (date < now)`);
